...
sunrise_watch_file = '/mnt/myDisk/shared/dayimages/IMAGE_0599.JPG'
days_end_file = '/mnt/myDisk/shared/dayimages/end.txt'

# sunset start file
def sunset_start(fpd):
    file_num = "/mnt/myDisk/shared/dayimages/IMAGE_" + str(fpd-450) + ".JPG"
    return file_num
...
def ffmpeger(first_image):
    if first_image == 0:
        ...
        folder = "sunset"
    day = strftime("%d-%b")
    video_filename = f"/mnt/myDisk/shared/timelapses/{day}-{folder}.mp4"
    push = pb.push_note("Running FFMPEG", "Well done")
    subprocess.call(f"/usr/local/bin/ffmpeg -y -r 30 -f image2 -start_number {start_file} -i /mnt/myDisk/shared/{folder}/IMAGE_%04d.JPG -vcodec libx264 -preset slow -crf 17 {video_filename}",shell = True)
    ...
    while running:
        ...
        if latest_file == sunrise_watch_file:
            ...
            for i in list_of_files:
                filename = i[29:]
                subprocess.call(f"cp {i} /mnt/myDisk/shared/sunrise/{filename}", shell=True)
            ffmpeger(0)
            sleep(10)
            ...
            first_image = fpd-450
            for i in range(450):
                current_filepath = "/mnt/myDisk/shared/dayimages/IMAGE_" + str(first_image + i) + ".JPG"
                current_filename = "IMAGE_" + str(first_image + i) + ".JPG"
                subprocess.call(f"cp {current_filepath} /mnt/myDisk/shared/sunset/{current_filename}", shell=True)
